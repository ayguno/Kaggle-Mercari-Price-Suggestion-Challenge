...
It is questionable how much diference this transformation adds in addition to log transformation.The variance decreases, which could be a good or bad thing.

Let's try to figure out by training new models after BoxCox transformation.


# Training algorithms after BoxCox transformation of the price:
...
lambda.mini.train <- BoxCox.lambda(mini.train$log.price)
mini.train<- mini.train %>% mutate(BoxCox.log.price = BoxCox(log.price,lambda = lambda.mini.train)) %>% select(-log.price)

```


### Extreme gradient boosting after BoxCox transformation
...
#Train the model
set.seed(12345)
xGBM.BC <- train(BoxCox.log.price ~ ., data = mini.train,method = "xgbLinear", trControl = fitControl, verbose = FALSE) 
#save the model for future use
saveRDS(xGBM.BC,"xGBM.BC.rds")
...
```{r}
# reload the saved model
xGBM.BC <- readRDS("xGBM.BC.rds")
log.rmse.training(pred = predict(xGBM.BC,mini.train),observed = mini.train$BoxCox.log.price,method = "BoxCox Extreme Gradient Boosting")
```

This performance looks great! Particularly RMSE at this scale is substantially reduced. 

Still not sure if this is a deceiving result because we are working at an artificial scale. But we will check this once we wil inverse BoxCox transform our predicted and expected results and calculate RMSE once again.

Overall promising results!

```{r}
pred.log <- InvBoxCox(predict(xGBM.BC,mini.train),lambda = lambda.mini.train)
obs.log <- InvBoxCox(mini.train$BoxCox.log.price,lambda = lambda.mini.train)
log.rmse.training(pred = pred.log,observed = obs.log,method = "InvBoxCox Extreme Gradient Boosting")

```

When we get back to the log transformed state, we indeed realize that the RMSE increases again. Therefore, the performance of the model looks infilated in the BoxCox transformed state of the response variable.


### Random Forest after BoxCox transformation
...
#Train the model
set.seed(12345)
RF.BC <- train(BoxCox.log.price ~ ., data = mini.train,method = "rf", trControl = fitControl, verbose = FALSE) 
#save the model for future use
saveRDS(RF.BC,"RF.BC.rds")
...
```{r}
# reload the saved model
RF.BC <- readRDS("RF.BC.rds")
log.rmse.training(pred = predict(RF.BC,mini.train),observed = mini.train$BoxCox.log.price,method = "BoxCox Random Forest")
```


```{r}
pred.log <- InvBoxCox(predict(RF.BC,mini.train),lambda = lambda.mini.train)
obs.log <- InvBoxCox(mini.train$BoxCox.log.price,lambda = lambda.mini.train)
log.rmse.training(pred = pred.log,observed = obs.log,method = "InvBoxCox Random Forest")

```


For Random Forest the same situation appears to be the case. 

Let's also try using BoxCox transformation on the predictors when trainign the model and see if that makes any difference:
...
#Train the model
set.seed(12345)
xGBM.BC2 <- train(BoxCox.log.price ~ ., data = mini.train,method = "xgbLinear",preProcess = "BoxCox", trControl = fitControl, verbose = FALSE) 
#save the model for future use
saveRDS(xGBM.BC2,"xGBM.BC2.rds")
...
```{r}
# reload the saved model
xGBM.BC2 <- readRDS("xGBM.BC2.rds")
log.rmse.training(pred = predict(xGBM.BC2,mini.train),observed = mini.train$BoxCox.log.price,method = "BoxCox Extreme Gradient Boosting BoxCox on predictors")
```



```{r}
pred.log <- InvBoxCox(predict(xGBM.BC2,mini.train),lambda = lambda.mini.train)
obs.log <- InvBoxCox(mini.train$BoxCox.log.price,lambda = lambda.mini.train)
log.rmse.training(pred = pred.log,observed = obs.log,method = "InvBoxCox Extreme Gradient Boosting")

```

It doesn't take too long to figure out that it doesn't make to mmuch difference.



Saving data sets for Deep Learning Experiments using Python:
...
write.csv(mini.subtrain,"mini_subtrain.csv")
write.csv(subtrain,"subtrain.csv")
