...
####Feature engineering through price segmentation
...
merged.table <- parse_text(mini.subtrain$item_description[mini.subtrain$price == 0])[1:10,c(1,3)]
...
for(i in seq_along(percentiles)){
        ...
        text <- mini.subtrain.nonzero$item_description[mini.subtrain.nonzero$price >= quantile(mini.subtrain.nonzero$price,percentiles[i])]
        ...
        # # Find the words that are unique in the current set compared to previous set
        # words.sorted.table <- as.character(sorted.table$Word)
        # words.merged.table <- as.character(merged.table$Word)
        # # find the sorted.table containing only the unique words
        # unique.words <- words.sorted.table[!(words.sorted.table %in% words.merged.table)]
        # sorted.table.unique <- sorted.table[(as.character(sorted.table$Word) %in% unique.words),]
        # Get the Top50 of the sorted.table and merge with the merged.table to accumulate
        merged.table <- merge(merged.table,head(sorted.table,50), by = "Word", all = TRUE)
        # continue until all price segments finished
} 
